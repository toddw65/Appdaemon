...
class Presence(appapi.AppDaemon):
  ...
  def initialize(self):
    ...
    self.set_state("sensor.todd_tracker", state = "away")
    self.set_state("sensor.angela_tracker", state = "away")
  ...
  def presence_event(self, event_name, data, kwargs):
    ...
    tracker = "Unknown"
    if event_tracker == "Todd":
        tracker = globals.todd_tracker_id
    elif event_tracker == "Angela":
        tracker = globals.angela_tracker_id
